...
> **Note du 24/09/2026 (décision PO, implémentation TypeScript `frontend/domain/domain.ts` uniquement).** Le barème collectif ci-dessus est remplacé : 1er 15 points, 2e 10, 3e 5, 4e 4, 5e 3, 6e et au-delà 1. Comptent chaque place de finale de catégorie **et** chaque place de toutes catégories (overall par discipline, champion seul confirmé compris) ; chaque inscription rapporte (plus de déduplication par personne) ; un inscrit confirmé éliminé avant la finale rapporte 1 point de participation. Départage inchangé (nombre de 1res places, puis 2es… puis critère personnalisé ; égalité persistante = même rang). `backend/fibda/domain.py` (plan B) garde l'ancienne règle : voir OPEN-QUESTIONS.md P11. L'overall final toutes disciplines n'est pas compté en attendant une réponse du PO.
