...
def validate_config(args):
    ...
    if not is_loopback(args.host) and not (args.cert and args.key):
    ...
    if public_url:
        ...
        if parsed.scheme != 'https' or not parsed.hostname or parsed.username or parsed.password or parsed.query or parsed.fragment or parsed.path not in ('', '/') or (parsed.port or 443) != args.port:
...
def diagnostics(directory, host, port):
    check_sqlite(host)
...
def main(argv=None):
    ...
    try:
        ...
        diagnostics(directory,args.host,args.port)
    except (ValueError,OSError,ssl.SSLError) as exc:
        parser.error(str(exc))
    from fibda.app import create_app
    import uvicorn
    application = create_app(directory,demo=args.demo)
    ...
    uvicorn.run(application,host=args.host,port=args.port,ssl_certfile=str(args.cert) if args.cert else None,ssl_keyfile=str(args.key) if args.key else None)
